	...
	return (
		<section className="panel">
			...
			<div className="policy-row" style={{ marginBottom: 12 }}>
				<button
					className={tab === "installed" ? "policy-chip active" : "policy-chip"}
					onClick={() => setTab("installed")}
				>
					Installed / Local
				</button>
				<button
					className={tab === "browse" ? "policy-chip active" : "policy-chip"}
				...
				{filtered.map((m) => (
					<div key={m.name} className="spec-card">
						<div className="mesh-card-head">
							...
							<span
								className={`mesh-status ${m.status === "Online" ? "online" : m.status === "Busy" ? "offline" : "offline"}`}
							>
								● {m.status}
							</span>
						</div>
