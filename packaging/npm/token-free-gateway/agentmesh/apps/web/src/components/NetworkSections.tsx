...
export function Trending() {
  const [items, setItems] = useState<
    { id: string; question: string; score: number }[]
  >([]);
  useEffect(() => {
...
export function HumanWanted() {
  const [items, setItems] = useState<
    { id: string; prompt: string; shares: number }[]
  >([]);
  const [done, setDone] = useState<Set<string>>(new Set());
  useEffect(() => {
    fetch(`${API}/api/human-wanted`)
      .then((r) => r.json())
      .then(setItems)
      .catch(() => {});
  }, []);
      ...
      <div className="versus-grid">
        {items.map((item) => (
          <article className="versus-card" key={item.id}>
            ...
            <span
              className={item.winner === "human" ? "winner human" : "winner ai"}
            >
  ...
  async function submit(event: { preventDefault(): void }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus(
          `💡 지식 후보로 등록되었습니다. 검증 후 +${submission.reward} Credit가 지급됩니다.`,
        );
        setContent("");
      } else {
