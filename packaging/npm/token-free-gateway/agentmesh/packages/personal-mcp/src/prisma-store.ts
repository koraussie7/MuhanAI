...
export class PrismaUserKnowledgeStore {
  ...
  async listKnowledge(
    ...
    const rows = await this.db.knowledgeNode.findMany({
      where: {
        ...
        ...(filter?.categoryId
          ? { categoryId: { startsWith: filter.categoryId } }
          : {}),
      },
      include: { permissions: true },
    ...
    await this.db.userStatsRow.update({
      ...
      data: {
        contributions: delta.contributions
          ? { increment: delta.contributions }
          : undefined,
        citations: delta.citations ? { increment: delta.citations } : undefined,
        helpfulness: delta.helpfulness
          ? { increment: delta.helpfulness }
          : undefined,
  ...
  private async recomputeStats(
    userId: string,
    opts?: { contributionsDelta?: number }
  ) {
    const [knowledgeCount, memoryCount, expertiseDomains, stats] =
      await Promise.all([
        this.db.knowledgeNode.count({ where: { ownerId: userId } }),
        this.db.userMemory.count({ where: { userId } }),
        this.db.userExpertise
          .groupBy({ by: ["domain"], where: { userId } })
          .then((g) => g.length),
