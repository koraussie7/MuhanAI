...
import type { AIProviderDescriptor, UserAIConnection, AIAnswer, AIRelayTask } from "./ai-provider.js";
...
export interface Evaluator {
  ...
  consensus(results: AgentResult[], threshold?: number): Promise<{ agreed: boolean; answer: AgentResult | undefined }>;
}
...
export interface HumanReviewer {
  register(actorId: string, capabilities: HumanCapability[], specialties?: string[], language?: string[]): Promise<void>;
  updateAvailability(actorId: string, available: boolean): Promise<void>;
  submitReview(actorId: string, requestId: string, result: Pick<ReviewResult, "rating" | "comment" | "approved">): Promise<void>;
...
export interface AIBridge {
  ...
  createRelay(input: Omit<AIRelayTask, "id" | "createdAt" | "updatedAt" | "status">): Promise<AIRelayTask>;
