...
export const CosmicCanvas: React.FC<CosmicCanvasProps> = ({
	...
	const cameraRef = useRef<{ x: number; y: number; zoom: number }>({
		x: 0,
		y: 0,
		zoom: getOptimalInitialZoom(),
	});
	const touchStateRef = useRef<any>({
		mode: "none",
		startX: 0,
		startY: 0,
		lastX: 0,
	...
	// removes the "tear down and rebuild the loop on every prop change"
	// hot path that previously amplified CPU cost.
	const packetStepRef = useRef(0);
	...
	// Sync prop inputs into the render-loop ref. The render closure is built
	// once on mount, so every frame must read inputs from this ref instead
	// of capturing them from React's render. This eliminates the
	// "tear-down-and-rebuild the rAF chain on every slider change" hot path
	// that previously amplified CPU cost.
	useEffect(() => {
	...
	useEffect(() => {
		...
		const render = (_dtMs: number, _profile: ReturnType<RenderScheduler["getProfile"]>) => {
			...
			const query = inputs.searchFilter.toLowerCase().trim();
			const filters = inputs.activeTypeFilters;
			...
			simEdges.forEach((edge, idx) => {
				...
				const isHighlighted =
					inputs.selectedNodeId === s.id ||
					inputs.selectedNodeId === t.id ||
			...
			simNodes.forEach((node) => {
				...
				const isSelected = inputs.selectedNodeId === node.id;
		...
		// profile as soon as the user touches anything on the page. Listeners
		// are passive — they MUST NOT block scrolling or interaction. Wired
		// at the document level so HUD button clicks, HUD slider drags, and
		// out-of-canvas activity all count as "user is still here".
		const bumpInteraction = () => scheduler.notifyInteraction();
	...
	const smoothFocusNode = (targetNode: CosmicNode) => {
		...
		const easeOutQuart = (x: number): number => 1 - (1 - x) ** 4;
