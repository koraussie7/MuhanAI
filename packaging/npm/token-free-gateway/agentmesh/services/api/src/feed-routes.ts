...
export async function feedRoutes(app: FastifyInstance) {
  ...
  app.get("/api/unsolved", async (_request, reply) => {
    ...
    try {
      ...
      return categories.map((c) => ({ category: c, label: labels[c], count: 0 }));
    }
  });
  ...
  app.get("/api/rewards/:actorId", async (request, reply) => {
    const { actorId } = request.params as { actorId: string };

    try {
      const prisma = await getPrisma();
      const answers = await prisma.helpAnswer.count({ where: { userId: actorId } });
      return {
        actorId,
        balance: answers * 120,
        entries: [],
