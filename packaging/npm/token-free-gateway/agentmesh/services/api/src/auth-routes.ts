...
export async function authRoutes(app: FastifyInstance) {
   ...
   */
  const memoryUsers = new Map<string, { id: string; email: string; name?: string; password: string }>();
  ...
  app.post("/api/auth/register", async (request, reply) => {
    ...
    try {
      ...
      memoryUsers.set(normalized, { id: userId, email: normalized, name, password });
  ...
  app.post("/api/auth/login", async (request, reply) => {
    ...
    try {
      ...
      return { token: createToken(user), user: { id: user.id, email: user.email, name: user.name } };
    }
  });
