...
import { DOMAIN_CATEGORIES, DomainCategory, createCategoryContext } from "./category";
...
export class CategoryClassifier {
  private keywordMap: Record<DomainCategory, string[]> = {
    medical: ["증상", "병원", "의사", "약", "진단", "질병", "symptom", "doctor", "medicine", "diagnosis"],
    legal: ["계약", "소송", "변호사", "법률", "조항", "contract", "lawsuit", "lawyer", "legal", "clause"],
    tax: ["세금", "부가세", "법인세", "소득세", "tax", "vat", "corporate tax", "income tax"],
    finance: ["투자", "주식", "펀드", "대출", "금리", "investment", "stock", "fund", "loan", "interest"],
    "real-estate": ["부동산", "임대", "보증금", "매매", "아파트", "real estate", "rental", "deposit", "lease", "apartment"],
    business: ["사업", "회사", "창업", "경영", "전략", "business", "startup", "company", "strategy"],
    education: ["교육", "학교", "대학", "강의", "학습", "education", "school", "university", "course"],
    technology: ["코딩", "소프트웨어", "ai", "프로그래밍", "서버", "software", "programming", "server", "api"],
  ...
  classify(text: string, hint?: Partial<CategoryContext>): ClassificationResult {
    ...
    const jurisdictions =
      hint?.jurisdiction ??
      detectJurisdictionFromText(text);
    ...
    const riskLevel: RiskLevel =
      hint?.riskLevel ??
      globalTaxonomy.get(topDomain)?.defaultRiskLevel ??
      "medium";
