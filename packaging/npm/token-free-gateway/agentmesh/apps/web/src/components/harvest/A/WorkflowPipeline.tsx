...
const INITIAL_STAGES: PipelineStage[] = [
  { id: "intake", label: "쿼리 수신 & 복잡도 분석", role: "Router Node", assignedTo: "Local Gateway Router", status: "success", duration: "12ms" },
  { id: "search", label: "다중 에이전트 지식 검색", role: "Search Submesh", assignedTo: "Gemini + WebSearch", status: "success", duration: "1.1s" },
  { id: "consensus", label: "Agent Cast 4인 교차 합의", role: "Consensus Engine", assignedTo: "Claude + DeepSeek + Local", status: "running", duration: "처리 중..." },
  { id: "poh", label: "인간 전문가 검증 게이트", role: "PoH Validator", assignedTo: "Human Expert Pool", status: "idle" },
  { id: "store", label: "CRDT v2 지식 레이크 영구화", role: "Storage Mesh", assignedTo: "P2P Knowledge Pool", status: "idle" },
];
...
export function WorkflowPipeline() {
  ...
  const advancePipeline = () => {
    setStages((prev) => {
      ...
      if (runningIdx === -1) {
        setIsRunning(true);
        return prev.map((n, i) => (i === 0 ? { ...n, status: "running", duration: RUNNING_DURATION } : { ...n, status: "idle" }));
      }
      const isLast = runningIdx === prev.length - 1;
      setIsRunning(!isLast);
      return prev.map((n, i) => {
        if (i === runningIdx) return { ...n, status: "success", duration: DEFAULT_ADVANCE_DURATION };
    ...
    <div className="workflow-pipeline">
      <div className="peer-canvas-toolbar workflow-pipeline__toolbar">
        <div className="workflow-pipeline__heading">
          <span className="a2a-dot" style={{ background: isRunning ? "var(--accent-lime, #e6ff87)" : "var(--text-muted, #888)" }} />
        ...
        <div className="workflow-pipeline__column">
          {stages.map((stage, idx) => (
            <React.Fragment key={stage.id}>
              ...
              {idx < stages.length - 1 && (
                <div className="workflow-pipeline__connector" aria-hidden="true">↓</div>
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="workflow-pipeline__card-title">
          <span
            className={`webrtc-dot ${
              stage.status === "success" ? "connected"
              : stage.status === "running" ? "connecting"
              : "offline"
            }`}
          />
      ...
      <div className="workflow-pipeline__card-meta">
        <span
          className={`mesh-status ${
            stage.status === "success" ? "online"
            : stage.status === "running" ? "busy"
            : "offline"
          }`}
        >
          {stage.status === "success" ? "✓ 완료"
            : stage.status === "running" ? "● 처리 중"
            : stage.status === "error" ? "✗ 오류"
            : "○ 대기"}
        </span>
        {stage.duration && (
          <div className="workflow-pipeline__duration">{stage.duration}</div>
        )}
      </div>
    </article>
  );
