...
import { getMenuTranslation, MENU_TRANSLATIONS, type NavI18n } from "./menu-i18n.js";
...
test("menu-i18n: all 8 languages have complete group translations", () => {
	for (const lang of SUPPORTED_LANG_CODES) {
		...
		for (const group of NAV_GROUPS) {
			assert.ok(trans.groups[group.id], `${lang}: missing group '${group.id}'`);
			for (const item of group.items) {
				assert.ok(trans.items[item.id], `${lang}: item '${item.id}' in group '${group.id}'`);
			}
		}
	}
});
...
test("menu-i18n: desktop section has all required keys", () => {
	for (const lang of SUPPORTED_LANG_CODES) {
		const trans = getMenuTranslation(lang);
		assert.ok(trans.desktop, `${lang}: desktop section should exist`);
		assert.ok(trans.desktop.searchPlaceholder, `${lang}: searchPlaceholder`);
		...
		for (const app of ["chat", "terminal", "engine", "editor", "settings", "bitterbot"]) {
			assert.ok(trans.desktop.apps?.[app], `${lang}: app '${app}'`);
			assert.ok(trans.desktop.apps?.[app]?.name, `${lang}: app '${app}'.name`);
			assert.ok(trans.desktop.apps?.[app]?.description, `${lang}: app '${app}'.description`);
		}
	}
});
...
test("menu-i18n: Korean translations are non-empty and contain Hangul", () => {
	...
	for (const group of NAV_GROUPS) {
		assert.ok(ko.groups[group.id], `ko: missing group '${group.id}'`);
		for (const item of group.items) {
			if (ko.items[item.id]) {
				assert.ok(
	...
	assert.ok(/[\uAC00-\uD7A3]/.test(ko.desktop?.allApps || ""), "ko: allApps should be Hangul");
	assert.ok(
...
test("i18n: detectInitialLanguage returns valid language", () => {
	...
	assert.ok(SUPPORTED_LANG_CODES.includes(lang), `language '${lang}' should be supported`);
});
