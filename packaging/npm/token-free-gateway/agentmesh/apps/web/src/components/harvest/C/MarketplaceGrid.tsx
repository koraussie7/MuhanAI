...
const AGENTS: MarketAgent[] = [
	{
		...
		caps: ["Research", "Web Search", "Summarization"],
		price: 80,
		unit: "query",
		rating: 4.8,
	},
	{
		id: "a-2",
		name: "Claude Analysis Pro",
		caps: ["Analysis", "Long-form", "Coding"],
		price: 120,
		unit: "query",
		rating: 4.9,
	},
	...
	{
		id: "a-4",
		name: "Llama Local Assist",
		caps: ["Local", "Offline", "Privacy"],
		price: 0,
		unit: "query",
		rating: 4.3,
	},
	{
		...
		caps: ["Fact-check", "Source vs Source", "Consensus"],
		price: 60,
		unit: "query",
		rating: 4.6,
	},
...
const EXPERTS: MarketExpert[] = [
	...
	},
	{
		id: "e-2",
		name: "Sarah Chen",
		expertise: ["AI Regulation", "EU AI Act"],
		creditsHr: 320,
		rating: 4.8,
		responseMin: 68,
	},
	...
	},
	{
		id: "e-4",
		name: "Alex Rivera",
		expertise: ["MCP Integration", "Tooling"],
		creditsHr: 280,
		rating: 4.6,
		responseMin: 55,
	},
...
function Stars({ value }: { value: number }) {
	return (
		<span className="hc-stars" aria-label={`${value} / 5`}>
			{[1, 2, 3, 4, 5].map((n) => (
				<span key={n} className={n <= Math.round(value) ? "hc-star on" : "hc-star"}>
					★
				</span>
			))}
			...
			{AGENTS.map((a) => (
				<MarketCard key={a.id}>
					...
					<div className="hc-mkt-caps">
						{a.caps.map((c) => (
							<span key={c} className="hc-mkt-cap">
								{c}
							</span>
						))}
					</div>
					<div className="hc-mkt-foot">
						<span className="hc-mkt-price">
							{a.price === 0 ? "무료" : `${a.price} cr/${a.unit}`}
						</span>
		...
		<div className="hc-mkt-grid">
			{EXPERTS.map((e) => (
				<MarketCard key={e.id}>
					...
					<div className="hc-mkt-caps">
						{e.expertise.map((x) => (
							<span key={x} className="hc-mkt-cap">
								{x}
							</span>
						))}
					</div>
						...
						<div className="hc-worker-load">
							<div className="hc-load-bar">
								<div
									...
									style={{ width: `${Math.round(node.load * 100)}%` }}
								/>
							</div>
