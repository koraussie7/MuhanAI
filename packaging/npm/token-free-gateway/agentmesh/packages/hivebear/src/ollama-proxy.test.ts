...
import { OllamaProxy, defaultGuard } from "./ollama-proxy.js";
...
describe("OllamaProxy SSRF guard", () => {
  describe("defaultGuard", () => {
    ...
    it("rejects hostname that cannot be resolved", async () => {
      const result = await defaultGuard.validate(
        "http://does-not-exist.invalid:11434",
      );
      expect(result.ok).toBe(false);
  ...
  describe("OllamaProxy constructor (sync literal check)", () => {
    ...
    it("throws on a literal private IPv4 target", () => {
      expect(() => new OllamaProxy("http://10.0.0.5:11434")).toThrow(
        /private|reserved/i,
      );
    });
    ...
    it("throws on the literal metadata endpoint", () => {
      expect(() =>
        new OllamaProxy("http://169.254.169.254/latest/meta-data"),
      ).toThrow(/private|reserved/i);
    });

    it("allows private targets when allowPrivateTargets is true", () => {
      expect(
        () => new OllamaProxy("http://10.0.0.5:11434", { allowPrivateTargets: true }),
      ).not.toThrow();
    });

    it("does not throw on hostnames (resolved at request time)", () => {
      expect(
        () => new OllamaProxy("http://my-public-host.example.com:11434"),
      ).not.toThrow();
    });
  ...
  describe("handler end-to-end", () => {
    ...
    function makeRequest(port: number, path: string): Promise<{
      status: number;
      body: string;
          ...
          (res) => {
            ...
            res.on("end", () =>
              resolve({
                ...
                requestId:
                  (res.headers["x-request-id"] as string | undefined) ?? null,
              }),
            );
          },
        );
