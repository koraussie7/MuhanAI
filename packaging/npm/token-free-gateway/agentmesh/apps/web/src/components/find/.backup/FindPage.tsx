...
export const FindPage: React.FC<FindPageProps> = ({ onNavigateHome }) => {
	...
	const [peers, _setPeers] = useState<PeerInfo[]>(INITIAL_PEERS);
	...
	const spawnShockwave = useCallback(
		(sx: number, sy: number, color = "#38bdf8") => {
	...
	const handleSearchOrPublish = useCallback(
		(query: string) => {
			...
			const newNode: CosmicNode = {
				...
				label: `[[${clean}.md]]`,
				type: "note",
				peerId: userPeerConnected
					? "peer-local-user-browser"
					: "peer-anonymous-creator",
				x: nx,
				y: ny,
				vx: 0,
				vy: 0,
			...
			logEvent(
				`✨ Published new Obsidian node: [[${clean}.md]] into Cosmic Mesh`,
			);
		},
	...
	const handleConnectUserPeer = useCallback(() => {
		setUserPeerConnected((prev) => {
			...
			logEvent(
				next
					? "user device mounted into P2P mesh"
					: "user device disconnected from P2P mesh",
			);
			return next;
		});
		setLatencyMs((prev) =>
			Math.max(1, Math.round(prev + (Math.random() - 0.5) * 12)),
		);
		playCosmicChime();
	}, [logEvent]);
