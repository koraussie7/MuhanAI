	...
	const [votes] = useState<AgentVote[]>([
		{
			...
			decision: "agree",
			confidence: 98,
			reason: "WebRTC DataChannel 128B MTU 조각화 방지 증명 검증 완료",
		},
		{
			...
			decision: "agree",
			confidence: 96,
			reason: "RFC 8831 WebRTC Data Channels 표준 문서 인용 일치",
		},
		{
			agent: "DeepSeek R1",
			avatar: "⚡",
			decision: "agree",
			confidence: 92,
			reason: "수학적 지연 시간 O(log N) 가십 알고리즘 증명 확인",
		},
		{
			agent: "Local Llama-3.3",
			avatar: "💻",
			decision: "modify",
			confidence: 78,
			reason: "ARM64 환경에서 SIMD 가속시 메모리 복사 오버헤드 주석 권고",
		},
	]);

	const [activeAgent, setActiveAgent] = useState<AgentVote>(
		votes[0] ?? {
			agent: "Claude",
			avatar: "🤖",
			decision: "agree",
