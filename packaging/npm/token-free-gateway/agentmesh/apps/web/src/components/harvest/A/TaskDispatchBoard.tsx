    ...
    <div className="task-dispatch-container">
      ...
      <div className="policy-row" style={{ marginTop: 14, marginBottom: 14 }}>
        {[
          ...
          { key: "queued", label: `대기 (${tasks.filter((t) => t.status === "queued").length})` },
          { key: "dispatching", label: `배정 중 (${tasks.filter((t) => t.status === "dispatching").length})` },
          { key: "running", label: `실행 중 (${tasks.filter((t) => t.status === "running").length})` },
          { key: "completed", label: `완료 (${tasks.filter((t) => t.status === "completed").length})` },
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      <div className="peer-grid">
        {filtered.map((task) => (
          <article className="peer-card" key={task.id} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div className="peer-card-head">
              ...
              />
              <strong className="peer-name" style={{ fontSize: 13 }}>{task.id}</strong>
              <span
            ...
            <p style={{ fontSize: 12, color: "#8f9188", margin: 0, lineHeight: 1.4 }}>{task.description}</p>

            <div className="peer-card-meta" style={{ marginTop: 6 }}>
              <span>담당: <strong style={{ color: "#d1d4c7" }}>{task.assignedAgent}</strong></span>
              <span className="peer-latency">+{task.credits} CR</span>
            </div>

            {/* Progress Bar */}
            <div style={{ background: "#22251d", borderRadius: 3, height: 6, overflow: "hidden", marginTop: 4 }}>
              <div
                style={{
                  width: `${task.progress}%`,
                  height: "100%",
            ...
            <div className="peer-card-foot" style={{ marginTop: 8 }}>
              ...
              {task.status !== "completed" ? (
                ...
                >
                  {task.status === "queued" ? "배정 실행 ▶" : task.status === "dispatching" ? "실행 시작 ▶" : "완료 처리 ✓"}
                </button>
              ) : (
