...
import { z } from "zod";
import { wheelProtocol } from "@agentmesh/knowledge-base";
import type { SignedRecord, FederationNode } from "@agentmesh/knowledge-base";
...
export async function knowledgeRoutes(app: FastifyInstance) {
  ...
  app.post("/api/knowledge/wheel/register", async (request, reply) => {
    ...
    wheelProtocol.register({ id, type, content, ownerId, shared, timestamp: Date.now() });
  ...
  app.get("/api/knowledge/wheel/shared", async (_request, reply) => {
    const shared = wheelProtocol.listShared();
    return { components: shared };
  });

  app.post("/api/knowledge/wheel/share/:id", async (request, reply) => {
  ...
  app.get("/api/knowledge/folklore/peers", async (_request, reply) => {
  ...
  app.delete("/api/knowledge/folklore/peers/:peerId", async (request, reply) => {
    const { peerId } = request.params as { peerId: string };
    federation.removePeer(peerId);
    return { removed: true, peerId };
  });

  app.post("/api/knowledge/folklore/sync", async (_request, reply) => {
