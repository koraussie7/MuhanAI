...
const endpoints = [
  {
    ...
    fetch: () =>
      fetch(`${process.env.OMNIROUTE_BASE_URL ?? "http://localhost:20128/v1"}/chat/completions`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: process.env.OMNIROUTE_API_KEY ? `Bearer ${process.env.OMNIROUTE_API_KEY}` : `Bearer ${process.env.OPENROUTER_API_KEY ?? "local"}` },
        body: JSON.stringify({ model: process.env.OMNIROUTE_MODEL ?? "auto", messages: [{ role: "user", content: PROMPT }], temperature: 0.2, max_tokens: 16 }),
  ...
  },
  {
    ...
    fetch: () =>
      fetch("http://127.0.0.1:3456/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "claude-3-7-sonnet", messages: [{ role: "user", content: PROMPT }] }),
  ...
  },
  {
    ...
    fetch: () =>
      fetch("https://openrouter.ai/api/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "HTTP-Referer": "https://muhanai.com", "X-Title": "MuhanAI Token-Free Gateway" },
        body: JSON.stringify({ model: "meta-llama/llama-3.3-70b-instruct:free", messages: [{ role: "user", content: PROMPT }], max_tokens: 16 }),
  ...
  },
  {
    ...
    fetch: () =>
      fetch("https://text.pollinations.ai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "openai-fast", messages: [{ role: "system", content: SYSTEM }, { role: "user", content: PROMPT }], stream: false, max_tokens: 16 }),
        signal: AbortSignal.timeout(20000),
      }),
    parse: async (res) => (await res.json()).choices?.[0]?.message?.content ?? "",
  },
  {
    ...
    fetch: () =>
      fetch(`https://text.pollinations.ai/prompt/${encodeURIComponent(`${SYSTEM}\n\n${PROMPT}`)}?model=openai-fast`, { method: "GET", signal: AbortSignal.timeout(20000) }),
  ...
  },
  {
    ...
    fetch: () =>
      fetch("https://api-inference.huggingface.co/models/Qwen/Qwen2.5-0.5B-Instruct", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ inputs: `<|im_start|>user\n${PROMPT}<|im_end|>\n<|im_start|>assistant\n`, options: { wait_for_model: true } }),
  ...
  },
  {
    ...
    fetch: () =>
      fetch("https://api.cloudflare.com/client/v4/ai/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "@cf/meta/llama-3.2-3b-instruct", messages: [{ role: "user", content: PROMPT }] }),
        signal: AbortSignal.timeout(15000),
      }),
...
for (const ep of endpoints) {
  ...
  try {
    ...
    let text = "";
    try { text = (await ep.parse(res)).trim().slice(0, 120); } catch { text = "(parse failed)"; }
