...
const eventSubscribers = new Map<string, Set<(event: A2UIEvent) => void>>();
const allowedActions = new Set(["request_reservation"]);

function publish(event: A2UIEvent): void {
...
export async function a2uiRoutes(app: FastifyInstance) {
	...
	if (!surfaceId || !name) return reply.code(400).send({ error: "surfaceId and name are required" });
	...
	const send = (event: A2UIEvent) => reply.raw.write(`event: ${event.type}\ndata: ${JSON.stringify(event)}\n\n`);
