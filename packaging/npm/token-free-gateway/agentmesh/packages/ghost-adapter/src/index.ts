...
export function createGhostDiscoveryClient(fetchImpl: typeof fetch = fetch): GhostDiscoveryClient {
  return {
    async discover(baseUrl) {
      const url = new URL("/.well-known/agent.json", baseUrl).toString();
      const response = await fetchImpl(url, { headers: { accept: "application/json" } });
      if (!response.ok) throw new Error(`Ghost Agent Card request failed: ${response.status}`);
      const value: unknown = await response.json();
      return parseGhostAgentCard(value, baseUrl);
    },
  };
}
...
export function parseGhostAgentCard(value: unknown, fallbackUrl: string): GhostAgentCard {
  ...
  const capabilities = Array.isArray(record.capabilities)
    ? record.capabilities.filter((item): item is GhostCapability =>
  ...
  return {
    ...
    ...(record.privacy && typeof record.privacy === "object"
      ? { privacy: { filesStayLocal: (record.privacy as Record<string, unknown>).filesStayLocal === true } }
      : {}),
