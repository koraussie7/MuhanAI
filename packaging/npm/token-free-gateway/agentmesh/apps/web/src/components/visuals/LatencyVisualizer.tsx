...
const INITIAL_ROUTES: RouteLatency[] = [
  { provider: "WebLLM (Local)", route: "Browser Memory ➔ WebGPU", latencyMs: 14, status: "optimal", costTier: "free" },
  { provider: "P2P Mesh Node (KR)", route: "WebRTC DataChannel #42", latencyMs: 28, status: "optimal", costTier: "free" },
  { provider: "FreeLLMAPI Gateway", route: "HTTP/2 WebSession Stream", latencyMs: 185, status: "acceptable", costTier: "free" },
  { provider: "Groq LPU Engine", route: "Direct Cloud API (Tier 2)", latencyMs: 142, status: "acceptable", costTier: "low" },
  { provider: "Anthropic Claude 3.5", route: "Zero-Trust Failover Vault", latencyMs: 840, status: "degraded", costTier: "paid", isFailover: true },
];
          ...
          return (
            <div className="latency-route-row" key={r.provider}>
              <div className="latency-provider-info">
                <span className={`status-dot ${r.status === "optimal" ? "online" : r.status === "acceptable" ? "busy" : "offline"}`} />
