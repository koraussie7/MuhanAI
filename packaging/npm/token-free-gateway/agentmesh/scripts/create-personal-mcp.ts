...
async function main() {
	...
	const { userId, uko, mcp, created } = await createPersonalMcpForUser("user_minh_001", {
		profile: {
			name: "Minh",
	...
	const kn2 = await addUserKnowledge(userId, {
		title: "VN-KR 이중과세 방지",
		content: "한-베트남 조세조약 적용 시 거주자 판정과 원천징수 세율이 핵심이다.",
	...
	const context = await buildPersonalContext(userId, "다낭 보증금 반환 어떻게 하나요?");
	...
	console.log(
		"expertise:",
		final?.expertise.map((e) => `${e.domain}/${e.subdomain ?? "-"}`),
	);
}
