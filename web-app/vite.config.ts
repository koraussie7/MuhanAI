 ...
 */
export function dashboardV2Plugin(): Plugin {
	const source = new URL("./public/dashboard-v2.html", import.meta.url);
	return {
		...
		configureServer(server) {
			server.middlewares.use(async (req, res, next) => {
				...
				try {
					...
					res.end(
						`dashboard-v2.html not found: ${err instanceof Error ? err.message : "unknown"}`,
					);
				}
			});
		},
...
/**
 * Rollup resolves every id listed in `manualChunks` as a build entry and fails
 * the whole build when one cannot be resolved.
 *
 * `web-app` is also built from partial worktrees where the agentmesh workspace
 * packages are not linked into `web-app/node_modules`. A static list then makes
 * `vite build` fail *after* the module graph is transformed, leaving `dist/`
 * half-written: Vite has already emptied it and copied `public/`, but never
 * emitted the SPA bundles. The visible symptom is a `dist/` that contains only
 * `dashboard-v2.html` — so the muhanai.com/dashboard injection survives while
 * the React app it is served alongside silently disappears.
 *
 * Chunk grouping is a caching optimisation only, so unresolvable groups are
 * dropped (Rollup falls back to its default splitting) rather than breaking
 * the emit that the /dashboard deployment depends on.
 */
function resolvedManualChunks(): Record<string, string[]> {
	const groups: Record<string, string[]> = {
		"mcp-vendor": ["@agentmesh/personal-mcp", "@agentmesh/shared-types"],
		vendor: ["lucide-react"],
	};
	const resolved: Record<string, string[]> = {};
	for (const [name, specifiers] of Object.entries(groups)) {
		const installed = specifiers.filter((specifier) =>
			existsSync(join(process.cwd(), "node_modules", specifier, "package.json")),
		);
		if (installed.length > 0) resolved[name] = installed;
	}
	return resolved;
}

export default defineConfig({
	plugins: [react(), tailwindcss(), llmChatPlugin(), mcpRpcPlugin(), dashboardV2Plugin()],
	server: { port: 5173, proxy: { "/api": "http://localhost:3001" } },
	build: {
		outDir: "dist",
		rollupOptions: {
			output: {
				manualChunks: resolvedManualChunks(),
			},
		},
