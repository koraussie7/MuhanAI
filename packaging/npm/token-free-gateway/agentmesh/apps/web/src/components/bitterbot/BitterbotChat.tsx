...
export function BitterbotChat() {
  const [messages, setMessages] =
    useState<BitterbotMessage[]>(loadBitterbotHistory);
  const [input, setInput] = useState("");
  const [isGenerating, setIsGenerating] = useState(false);
  const [lastResponse, setLastResponse] = useState<BitterbotResponse | null>(
    null,
  );
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if ((cause as Error)?.name !== "AbortError")
        setError(
          cause instanceof Error ? cause.message : "Bitterbot request failed",
        );
    } finally {
      ...
      >
        {messages.length === 0 && (
          <div style={{ color: "#94a3b8", lineHeight: 1.6 }}>
            Ask Bitterbot anything. It tries local WebGPU, local
            OpenAI-compatible runtimes, and always returns an offline status
            response when inference is unavailable.
          </div>
        )}
        {messages.map((item) => (
          <div
          ...
          >
            <strong
              style={{ color: item.role === "user" ? "#38bdf8" : "#f59e0b" }}
            >
        ...
        {isGenerating && (
          <div style={{ color: "#f59e0b" }}>Bitterbot is thinking…</div>
        )}
        {error && (
