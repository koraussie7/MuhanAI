...
export class LLMRouter {
  ...
  async generate(req: LLMRequest): Promise<LLMResponse> {
    ...
    const requestedProvider = req.provider ?? (process.env.LLM_DEFAULT_PROVIDER as LLMProvider) ?? "openai";
    try {
  ...
  private async generateWithProvider(provider: LLMProvider, req: LLMRequest): Promise<LLMResponse> {
    switch (provider) {
      case "openai": return this.generateOpenAI(req);
      case "anthropic": return this.generateAnthropic(req);
      case "google": return this.generateGoogle(req);
      case "local": return this.generateLocal(req);
  ...
  private async generateGoogle(req: LLMRequest): Promise<LLMResponse> {
    ...
    const model = client.getGenerativeModel({ model: req.model ?? "gemini-1.5-pro" });
    ...
    return {
      ...
      usage: { inputTokens: result.response.usageMetadata?.tokenCount ?? 0, outputTokens: 0 },
      latencyMs: 0,
    };
  }
    ...
    try {
      const res = await fetch(endpoint + "/api/generate", {
        ...
        body: JSON.stringify({
          ...
          stream: false,
          options: { temperature: req.temperature ?? 0.3, num_predict: req.maxTokens ?? 4096 },
        }),
      });
      if (!res.ok) throw new Error("Local LLM error: " + res.status);
      const data = await res.json() as { response?: string };
      return { text: data.response ?? "", provider: "local", model: req.model ?? "llama3", usage: { inputTokens: 0, outputTokens: 0 }, latencyMs: 0 };
    } catch {
  ...
  private generateMock(provider: LLMProvider, req: LLMRequest): LLMResponse {
    ...
    return { text: lines.join("\n"), provider, model: req.model ?? provider + "-mock", usage: { inputTokens: 0, outputTokens: 0 }, latencyMs: 0 };
  }
  ...
  private async tryFallbacks(req: LLMRequest, exclude: LLMProvider[]): Promise<LLMResponse | null> {
    ...
    for (const provider of available) {
      try { return await this.generateWithProvider(provider, req); } catch { continue; }
    }
    return this.generateMock("local", req);
  }

  private normalizeError(err: unknown, provider: LLMProvider): Error & { provider: LLMProvider; retryable: boolean } {
  ...
  async generateWithFallback(req: LLMRequest): Promise<LLMResponse> {
    ...
    return { ...this.generateMock("local", req), latencyMs: Date.now() - start };
  }
}
