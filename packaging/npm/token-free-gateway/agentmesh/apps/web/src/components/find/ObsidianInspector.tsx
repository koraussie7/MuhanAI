...
export const ObsidianInspector: React.FC<ObsidianInspectorProps> = ({
	...
	const backlinks = allNodes.filter((other) => {
		...
		return (
			other.frontmatter.links.includes(node.id) || backlinkWikiLink.test(other.frontmatter.markdown)
		);
	});
	...
	const renderMarkdownBody = (markdownText: string) => {
		...
		return segments.map((seg, idx) => {
			if (seg.type === "code") {
				const inner = seg.content.replace(/^```[^\n]*\n?/, "").replace(/\n?```$/, "");
				return (
			...
			return parts.map((part, pidx) => {
				if (part.startsWith("[[") && part.endsWith("]]")) {
					...
					const targetNode = allNodes.find(
						(n) =>
							n.frontmatter.title.toLowerCase() === stripped || n.id.toLowerCase() === stripped,
					);
					...
					return (
						<button
							...
							title={targetNode ? `Jump to ${targetNode.frontmatter.title}` : `Link: ${linkText}`}
						>
		...
		<aside className="obsidian-inspector-drawer">
			...
			<div className="obsidian-inspector-header">
				<div className="flex items-center gap-2 overflow-hidden">
					...
					<span className="obsidian-tab-title truncate">{node.frontmatter.title}.md</span>
				</div>
			...
			<div className="obsidian-inspector-body">
				...
				<div className="obsidian-yaml-box">
					...
					{node.peerId && (
						<div className="obsidian-yaml-row">
							...
							<span className="yaml-val text-emerald-400 font-mono">{node.peerId}</span>
						</div>
					)}
					<div className="obsidian-yaml-row">
				...
				<div className="obsidian-summary-callout">
					<p className="text-xs text-slate-300 leading-relaxed font-sans">{fm.summary}</p>
				</div>
				...
				<div className="obsidian-backlinks-section">
					...
					{backlinks.length === 0 ? (
						<div className="text-xs text-slate-500 italic py-2">No linked mentions found</div>
					) : (
						<div className="space-y-1.5 mt-2">
							{backlinks.map((b) => (
								<div key={b.id} className="obsidian-backlink-item" onClick={() => onSelectNode(b)}>
					...
					<div className="obsidian-peer-card">
						<div className="flex items-center justify-between mb-2">
							<div className="flex items-center gap-1.5">
								...
								<span className="text-xs font-semibold text-slate-200">Peer Provenance</span>
							</div>
						...
						<div className="text-[11px] font-mono text-slate-400">
							Sync Status: <span className="text-emerald-400">Verified WebRTC Mesh Shard</span>
						</div>
					</div>
				)}
			</div>
