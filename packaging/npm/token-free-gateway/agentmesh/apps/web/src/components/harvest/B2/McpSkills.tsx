...
import { Check, Copy, Download, Terminal, Zap, Cpu, Sparkles } from "lucide-react";
...
export function McpSkills() {
  ...
  const [activeClientTab, setActiveClientTab] = useState<"claude" | "cline" | "cursor" | "curl">("cline");
      ...
      const res = await fetch("/api/mcp/rpc", {
        ...
        body: JSON.stringify({
          ...
          params: {
            ...
            arguments: { question: "Zero-Token Gateway CRDT merge protocol", query: "CRDT", title: "Test Note", content: "Test" },
          },
        }),
      });
      if (res.ok) {
    ...
    <section className="panel" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div className="section-heading compact" style={{ marginBottom: 0 }}>
          ...
          <h2>MuhanAI <em>MCP Skills & One-Click Gateway</em></h2>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 9999, background: "rgba(16, 185, 129, 0.15)", border: "1px solid rgba(16, 185, 129, 0.3)", color: "var(--cline-green)", fontSize: 11, fontFamily: "var(--font-mono, monospace)" }}>
      ...
      <div style={{ background: "rgba(8, 12, 23, 0.85)", border: "1px solid rgba(56, 189, 248, 0.3)", borderRadius: 14, padding: 18, boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Zap size={16} color="#38bdf8" />
            <span style={{ fontWeight: 700, fontSize: 13, color: "#f8fafc", letterSpacing: "0.02em" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button
              type="button"
              onClick={handleCopy}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 8, background: copied ? "rgba(16, 185, 129, 0.2)" : "rgba(255, 255, 255, 0.08)", border: copied ? "1px solid #10b981" : "1px solid rgba(255, 255, 255, 0.15)", color: copied ? "#10b981" : "#f8fafc", fontSize: 11.5, fontFamily: "var(--font-mono, monospace)", cursor: "pointer", transition: "all 0.18s" }}
            >
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 8, background: "rgba(255, 255, 255, 0.06)", border: "1px solid rgba(255, 255, 255, 0.12)", color: "#cbd5e1", fontSize: 11.5, fontFamily: "var(--font-mono, monospace)", cursor: "pointer" }}
            >
              <Download size={13} />
              <span>다운로드</span>
            </button>
          ...
          {(["cline", "claude", "cursor", "curl"] as const).map((tab) => (
            <button
              ...
              style={{
                ...
                border: activeClientTab === tab ? "1px solid rgba(56, 189, 248, 0.45)" : "1px solid transparent",
            ...
            >
              {tab === "cline" ? "Cline / Roo Code" : tab === "claude" ? "Claude Desktop" : tab === "cursor" ? "Cursor" : "cURL (API)"}
            </button>
          ))}
        </div>

        {/* Config Code Preview */}
        <pre style={{ margin: 0, padding: 12, borderRadius: 8, background: "#030611", border: "1px solid rgba(255, 255, 255, 0.06)", color: "#a5f3fc", fontSize: 11.5, fontFamily: "var(--font-mono, monospace)", overflowX: "auto", lineHeight: 1.5 }}>
      ...
      <div className="spec-grid">
        {filtered.map((tool) => (
          ...
          >
            ...
            <h4 className="help-question" style={{ color: "#38bdf8", fontFamily: "var(--font-mono, monospace)" }}>{tool.name}</h4>
            <p className="dash-note">{tool.description}</p>
            <div className="help-meta">
              <span>매개변수: {tool.parameters.length > 0 ? tool.parameters.map((p) => p.name).join(", ") : "없음"}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="help-card" style={{ marginTop: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <h4 className="help-question" style={{ margin: 0 }}>
              ⚡ 라이브 테스트 실행: <span style={{ color: "#38bdf8", fontFamily: "var(--font-mono, monospace)" }}>{selectedTool.name}</span>
            </h4>
            <button
              type="button"
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {selectedTool.parameters.map((param) => (
              <input
                key={param.name}
                className="ask-input"
                defaultValue={param.name === "question" ? "Zero-Token Gateway CRDT merge protocol" : param.name === "query" ? "CRDT" : "Title"}
          ...
          {result && (
            <div className="verify-queue-item" style={{ marginTop: 12, background: "rgba(16, 185, 129, 0.08)", borderColor: "rgba(16, 185, 129, 0.3)" }}>
              <span className="claim-id" style={{ color: "#34d399" }}>실행 결과 (JSON-RPC 2.0 Response)</span>
              <pre style={{ margin: "6px 0 0 0", fontFamily: "var(--font-mono, monospace)", fontSize: 11.5, color: "#f0fdf4", whiteSpace: "pre-wrap" }}>
                {result}
              </pre>
            </div>
          )}
