import type { BroadcastRequest, DownloadStatus, ModelSearchRequest } from "@agentmesh/noema";
...
function _sanitizeInput(input: string, maxLength = 1024): string {
...
export class NoemaService {
	...
	async searchManifests(req: ModelSearchRequest) {
		const result = await this.client.search(req.query, {
			limit: req.limit ?? 10,
		});
