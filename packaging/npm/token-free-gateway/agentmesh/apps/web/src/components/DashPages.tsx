...
import { type Peer, PeerCanvas, BrowserAgentPanel } from "./harvest/A/PeerCanvas.js";
import { type LocalNode, PersonalNodeControl } from "./harvest/A/PersonalNode.js";
import { type MeshNode, type MeshEdge, MeshTopology, MeshStats } from "./harvest/A/MeshTopology.js";

import { GatewayTable, PolicyChain, type GatewayProvider } from "./harvest/C/GatewayPanel.js";
import { LedgerTable, TrustRing, RewardChip, WebOfTrustBadge, type LedgerRow, type LedgerEntry } from "./harvest/C/index.js";
import { WorkerPool, type ComputeWorker } from "./harvest/C/WorkerPool.js";
import { GpuPool, type GpuNode } from "./harvest/C/GpuPool.js";
import { ClusterOverview, type ClusterNode } from "./harvest/C/ClusterView.js";
import "./harvest/C/harvest-c.css";
import {
  AgentsMarket, HumanExperts, McpMarket, KnowledgeMarket, ComputeMarket,
...
export function AgentMeshPage() {
  ...
  return (
    <Page title="Agent Mesh" subtitle="ISEK Agent Identity · A2A Discovery · DID Attestation · Decentralized Router">
      <SwarmRadar />
      <div className="peer-canvas-toolbar" style={{ marginBottom: 12 }}>
        <div className="peer-filter-row">
          <button className={`policy-chip ${view === "cards" ? "active" : ""}`} onClick={() => setView("cards")}>
            Identity Cards ({HARVEST_AGENTS.length})
          </button>
          <button className={`policy-chip ${view === "topology" ? "active" : ""}`} onClick={() => setView("topology")}>
            Mesh Topology
          </button>
        </div>
        {view === "cards" && (
...
const CHAIN_BY_POLICY: Record<string, string[]> = {
  "Free First": ["WebLLM (로컬 브라우저)", "FreeLLMAPI (무료 쿼터)", "P2P Compute (메시 공유)", "Paid API (최후 폴백)"],
  "Local First": ["WebLLM (로컬 브라우저)", "LocalAI / Ollama (셀프호스트)", "FreeLLMAPI (무료 쿼터)", "Paid API (최후 폴백)"],
};
...
export function LlmMeshPage() {
  ...
  return (
    <Page title="LLM Mesh & Gateway" subtitle={`tkngate 제로트러스트 라우팅 엔진 · 정책: ${policy}`}>
      <LatencyVisualizer />
...
export function TokenBankPage() {
  ...
  const [balance, setBalance] = useState<{ balance: number; entries: LedgerEntry[] } | null>(null);
  ...
  return (
    <Page title="Token Bank & Accounting" subtitle="p2ptokens 기여 회계 원장 · PinkyBrain Web of Trust 평판 링">
      <LiveBlockStream />
      <StatGrid stats={[["Balance", balanceValue], ["Entries", balance?.entries.length ?? 2]]} />
      <div className="hc-tokenbank-top" style={{ display: "flex", gap: 20, alignItems: "center", margin: "16px 0" }}>
...
const COMPUTE_WORKERS: ComputeWorker[] = [
  { id: "worker-se-01", role: "CPU Inference", load: 0.72, tasks: 128, status: "busy" },
  { id: "worker-gpu-02", role: "A100 · CUDA", load: 0.91, tasks: 342, status: "busy" },
  { id: "worker-web-03", role: "WebGPU Pool", load: 0.35, tasks: 87, status: "idle" },
  { id: "worker-edge-04", role: "Edge Node", load: 0.12, tasks: 15, status: "idle" },
  { id: "worker-us-05", role: "M · Metal", load: 0.58, tasks: 201, status: "busy" },
  { id: "worker-eu-06", role: "ROCm Train", load: 0, tasks: 0, status: "offline" },
];

const GPU_NODES: GpuNode[] = [
  { id: "gpu-a100-01", gpu: "NVIDIA A100 80GB", load: 0.91, vram: 80, online: true },
  { id: "gpu-m3-02", gpu: "Apple M3 Max", load: 0.58, vram: 36, online: true },
  { id: "gpu-4090-03", gpu: "RTX 4090", load: 0.74, vram: 24, online: true },
  { id: "gpu-7900-04", gpu: "Radeon 7900 XTX", load: 0, vram: 24, online: false },
];

const CLUSTER_NODES: ClusterNode[] = [
  { id: "coord-kr", type: "coordinator", gpu: "CPU", status: "healthy", utilization: 0.42 },
  { id: "gpu-a100-01", type: "worker", gpu: "A100 80GB", status: "healthy", utilization: 0.91 },
  { id: "gpu-m3-02", type: "worker", gpu: "M3 Max", status: "healthy", utilization: 0.58 },
  { id: "gpu-7900-04", type: "worker", gpu: "7900 XTX", status: "offline", utilization: 0 },
];
  ...
  return (
    <Page title="Compute Mesh" subtitle="AgentFM 분산 워커 · mycellm GPU 풀 · DAC 클러스터">
      ...
      <div className="policy-row" style={{ marginTop: 16, marginBottom: 16 }}>
        <button className={`policy-chip ${tab === "workers" ? "active" : ""}`} onClick={() => setTab("workers")}>
          ⚡ Compute Workers ({COMPUTE_WORKERS.length})
        </button>
        <button className={`policy-chip ${tab === "gpu" ? "active" : ""}`} onClick={() => setTab("gpu")}>
          🎮 GPU Pool ({GPU_NODES.length})
        </button>
        <button className={`policy-chip ${tab === "cluster" ? "active" : ""}`} onClick={() => setTab("cluster")}>
...
export function MarketplacePage({ defaultTab = "agents" as "agents" | "human" | "mcp" | "knowledge" | "compute" }: { defaultTab?: "agents" | "human" | "mcp" | "knowledge" | "compute" } = {}) {
  ...
  return (
    <Page title="Marketplace" subtitle="AI Capability App Store — 탈중앙화 자원 및 에이전트 마켓">
      <div className="policy-row" style={{ marginBottom: 16 }}>
        <button className={`policy-chip ${tab === "agents" ? "active" : ""}`} onClick={() => setTab("agents")}>🤖 Agents</button>
        <button className={`policy-chip ${tab === "human" ? "active" : ""}`} onClick={() => setTab("human")}>👤 Human Experts</button>
        <button className={`policy-chip ${tab === "mcp" ? "active" : ""}`} onClick={() => setTab("mcp")}>🔌 MCP Skills</button>
        <button className={`policy-chip ${tab === "knowledge" ? "active" : ""}`} onClick={() => setTab("knowledge")}>📚 Knowledge</button>
        <button className={`policy-chip ${tab === "compute" ? "active" : ""}`} onClick={() => setTab("compute")}>⚡ Compute</button>
      </div>
