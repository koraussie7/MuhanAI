...
describe("integrate", () => {
  ...
  test("pinned node velocity is zeroed, position frozen", () => {
    const n = makeNode({ id: "n", x: 500, y: 500, vx: 50, vy: 50, pinned: true });
    integrate([n], 1.0, null);
    assert.equal(n.vx, 0);
    assert.equal(n.vy, 0);
...
describe("step", () => {
  ...
  test("multiple nodes: O(n²) repulsion still terminates without NaN", () => {
    ...
    step(nodes, [], { repelStrength: 1.0, linkDistance: 100, centerGravity: 0.5, draggedNode: null });
...
describe("isNodeVisible", () => {
  const filters = { core: true, agent: true, peer: true, note: true, tag: true };
  ...
  test("query matches title", () => {
    const n = makeNode({ id: "n", frontmatter: { ...makeNode().frontmatter, title: "Cosmic Mesh" } });
    assert.equal(isNodeVisible(n, filters, "cosmic"), true);
  });

  test("query matches tag", () => {
    const n = makeNode({ id: "n", frontmatter: { ...makeNode().frontmatter, tags: ["alpha", "beta"] } });
  ...
  test("query with no match → hidden", () => {
    const n = makeNode({ id: "n", label: "Hello", frontmatter: { ...makeNode().frontmatter, title: "World", tags: [] } });
...
describe("screenToWorld", () => {
  test("identity at center, no camera offset, zoom=1", () => {
    assert.deepEqual(
      screenToWorld(500, 500, { cx: 500, cy: 500 }, { x: 0, y: 0, zoom: 1 }),
      { x: 0, y: 0 },
    );
  });
  ...
  test("zoom=2 shrinks the world units", () => {
    assert.deepEqual(
      screenToWorld(600, 500, { cx: 500, cy: 500 }, { x: 0, y: 0, zoom: 2 }),
      { x: 50, y: 0 },
    );
  });
  ...
  test("camera offset translates the world", () => {
    assert.deepEqual(
      screenToWorld(500, 500, { cx: 500, cy: 500 }, { x: 100, y: 50, zoom: 1 }),
      { x: -100, y: -50 },
    );
  });
});
