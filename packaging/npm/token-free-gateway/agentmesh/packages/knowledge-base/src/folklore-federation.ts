...
export class EnergyGate {
	...
	score(_queryEmbedding: number[], hits: Array<{ embedding: number[]; score: number }>): number {
...
export class FederationMesh {
	...
	async query(queryEmbedding: number[], queryText: string): Promise<SignedRecord[]> {
		const local = this.provenanceRanker.rank(
			Array.from(this.localRecords.values()),
			queryEmbedding,
		);
		...
		const admitted = hits.filter((_hit) => this.energyGate.shouldAdmit(queryEmbedding, hits));
	...
	async sync(): Promise<FederationSyncResult> {
		...
		for (const [peerId, peer] of this.peers) {
			...
			try {
				...
				const pushedCount = await this.transport.push(
					peerId,
					Array.from(this.localRecords.values()),
				);
				pushed += pushedCount;
