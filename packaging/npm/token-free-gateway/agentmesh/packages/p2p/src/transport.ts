...
export async function createTransport(
	cfg: TransportConfig,
): Promise<TransportResult<TransportHandle>> {
	...
	const transports: Array<
		| ReturnType<typeof tcp>
		| ReturnType<typeof webSockets>
		| ReturnType<typeof circuitRelayTransport>
	> = [tcp(), webSockets()];
	...
	try {
		...
		return {
			...
			isOk: () => false,
			error: makeErr(
				TransportErrorKind.InitFailed,
				`createLibp2p failed: ${(e as Error).message}`,
				e,
			),
	...
	node.addEventListener("peer:discovery", (evt) => {
		...
		const method =
			discovery.includes("bootstrap") && cfg.bootstrapPeers?.some((b) => b.includes(peerIdStr))
	...
	node.addEventListener("peer:connect", (evt) => {
		...
		void node.peerStore
			...
			.then((peer) => {
				const pubKey = (
					peer as unknown as {
						publicKey?: import("@libp2p/interface").PublicKey;
					}
				).publicKey;
				const now = Date.now();
				let positive = true;
	...
	try {
		...
		return {
			...
			isOk: () => false,
			error: makeErr(
				TransportErrorKind.ListenFailed,
				`node.start failed: ${(e as Error).message}`,
				e,
			),
			value: undefined as never,
		} as never;
