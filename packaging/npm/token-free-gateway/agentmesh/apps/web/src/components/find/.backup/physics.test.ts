...
describe("applyRepulsion", () => {
	...
	test("force scales with repelStrength", () => {
		...
		const vxHigh = a.vx;
		assert.ok(
			Math.abs(vxHigh) > Math.abs(vxLow),
			`2x repel should produce stronger force than 1x`,
		);
	});
	...
	test("force falls off with distance (1/r²-like)", () => {
		...
		assert.ok(
			Math.abs(closeVx) > Math.abs(farVx),
			`closer pair should repel more strongly`,
		);
	});
...
describe("applySprings", () => {
	...
	test("edge weight scales rest length", () => {
		...
		assert.ok(
			a.vx < 0,
			`a should be pushed away when rest length > dist (got ${a.vx})`,
		);
	});
...
describe("integrate", () => {
	test("applies damping each step", () => {
		...
		assert.ok(
			Math.abs(n.vx) > 0,
			`vx should still be nonzero mid-step (got ${n.vx})`,
		);
	});
	...
	test("nonzero gravity pulls node toward origin", () => {
		...
		assert.ok(
			n.vx < 0,
			`vx should become negative (toward origin, got ${n.vx})`,
		);
	});
...
describe("step", () => {
	test("composed: repulsion, springs, and integration in one call", () => {
		...
		assert.ok(
			a.x !== before.ax || a.y !== before.ay,
			`a should move after one step`,
		);
	});
