/**
 * Model Context Protocol (MCP) routes for MuhanAI.
 *
 * Ports the Cloudflare Worker-style handler in deploy/mcp-server.ts into a
 * Fastify plugin. Exposes the JSON-RPC 2.0 surface that the cosmic prompt bar
 * (apps/web/src/components/find/CosmicPromptBar.tsx) and external MCP clients
 * (Claude Desktop, Cline, Cursor) call to drive multi-agent quorum consensus,
 * the decentralized knowledge lake, and the pulse telemetry feed.
 *
 * Endpoints (mounted under server.ts):
 *   GET  /api/mcp/manifest.json       — MCP server declaration (also /.well-known/mcp.json)
 *   GET  /api/mcp/config              — one-click MCP client configurations
 *   POST /api/mcp/rpc                 — JSON-RPC 2.0 (tools/list, prompts/list,
 *                                        prompts/get, tools/call)
 *
 * The cosmic-prompt-bar flow does:
 *   POST /api/mcp/rpc { method: "tools/call", params: { name: "muhanai_ask_quorum", ... } }
 *
 * Until a real quorum provider is wired in, the call returns the same
 * consensus summary the cloudflare worker emits so the UI experience stays
...
const MUHANAI_MCP_PROMPTS: MCPPromptDefinition[] = [
	{
...
const MUHANAI_MCP_TOOLS: MCPToolDefinition[] = [
	{
...
> = {
	...
	es: {
		...
		bullet1: "• Un nuevo paradigma de coexistencia: Conectando humanos e IA hacia el infinito",
		...
		bullet5: "• Potenciando tus ideas con inteligencia autónoma para crear mayor valor juntos.",
		topicLabel: "Tema",
		footer:
	...
	fr: {
		...
		bullet1: "• Un nouveau paradigme de coexistence : Relier l'humain et l'IA vers l'infini",
...
function runToolCall(
	...
	if (toolName === "muhanai_search_knowledge") {
		return {
			content: JSON.stringify({
				query: args.query,
				matches: [
	...
	if (toolName === "muhanai_ask_quorum") {
	...
	if (toolName === "muhanai_publish_note") {
...
function buildPromptText(topic: string, requestedLang: string): string {
	const langData = PROMPT_LANG_MAP[requestedLang] ?? PROMPT_LANG_MAP.ko;
	if (!langData) return topic;
	return [
		langData.header,
		"",
		langData.bullet1,
...
export async function mcpRoutes(app: FastifyInstance): Promise<void> {
	...
	app.get("/.well-known/mcp.json", async (_req, reply) => {
		...
		return {
			...
			server_version: "1.0.0",
			rpc_endpoint: "/api/mcp/rpc",
			tools: MUHANAI_MCP_TOOLS.map((t) => t.name),
			prompts: MUHANAI_MCP_PROMPTS.map((p) => p.name),
		};
	});

	app.get("/api/mcp/config", async (req) => {
		const client =
			typeof (req.query as Record<string, unknown>).client === "string"
				? ((req.query as Record<string, unknown>).client as string)
				: "claude";

		const baseUrl = `${req.protocol}://${req.headers.host ?? "localhost"}`;
		const rpc = `${baseUrl}/api/mcp/rpc`;

		const claudeDesktopConfig = {
			mcpServers: {
				muhanai: {
					command: "npx",
					args: ["-y", "@agentmesh/mcp-server", "--gateway", baseUrl],
				},
			},
		};
	...
	app.post("/api/mcp/rpc", async (req, reply) => {
		...
		if (method === "tools/list") {
			return jsonRpcResponse(id, { tools: MUHANAI_MCP_TOOLS });
		}

		if (method === "prompts/list") {
			return jsonRpcResponse(id, { prompts: MUHANAI_MCP_PROMPTS });
		}

		if (method === "prompts/get") {
			const params = rpc.params ?? {};
			const args = (params.arguments ?? {}) as { topic?: string; language?: string };
			const topic =
				typeof args.topic === "string" && args.topic.length > 0
					? args.topic
					: "사람과 AI의 협업과 공존";
			const acceptLang =
				(req.headers["accept-language"] as string | undefined)?.slice(0, 2)?.toLowerCase() ?? "ko";
			const requestedLang = (args.language ?? acceptLang).toLowerCase();
			const promptText = buildPromptText(topic, requestedLang);
		...
		if (method === "tools/call") {
			...
			const outcome = runToolCall(toolName, args);
