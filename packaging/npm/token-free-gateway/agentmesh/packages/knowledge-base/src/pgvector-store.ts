...
import { EmbeddingService, embeddingService } from "./embeddings";
...
export class PgVectorStore {
   ...
   */
  async upsertNode(node: KnowledgeNode, chunks: { content: string; index: number }[]): Promise<void> {
  ...
  async search(query: RetrievalQuery): Promise<ScoredKnowledge[]> {
    ...
    const visibilityFilter =
      query.filter?.visibility && query.filter.visibility.length
