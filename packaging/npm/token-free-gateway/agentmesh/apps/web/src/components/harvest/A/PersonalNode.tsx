...
export function PersonalNodeControl({ node, onToggle, onModeChange }: {
  node: LocalNode;
  ...
  return (
    <div className="personal-node-card">
      <div className="personal-node-head">
        <span className={`status-dot ${running ? "online" : node.status === "error" ? "offline" : "idle"}`} />
      ...
      <div className="personal-node-meta">
        ...
        <div className="mode-switch">
          {(["local", "browser", "gateway"] as const).map(m => (
            <button key={m} className={`policy-chip sm ${node.mode === m ? "active" : ""}`} onClick={() => onModeChange?.(m)}>{m}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mesh-actions">
        <button className={running ? "btn-secondary" : "btn-primary"} onClick={() => onToggle?.(!running)}>
          {running ? "Pause" : "Start"}
        </button>
...
export function LocalAgentNodeList({ nodes, onSelect }: {
  nodes: LocalNode[];
  onSelect?: (id: string) => void;
}) {
  const [q, setQ] = useState("");
  const filtered = q ? nodes.filter(n => n.label.toLowerCase().includes(q.toLowerCase()) || n.id.toLowerCase().includes(q.toLowerCase())) : nodes;
  return (
    <div className="local-node-list">
      <div className="local-node-search">
        <input className="search-input" placeholder="노드 검색…" value={q} onChange={e => setQ(e.target.value)} />
      ...
      <div className="local-node-grid">
        {filtered.map(n => (
          <button key={n.id} className="local-node-row" onClick={() => onSelect?.(n.id)}>
            <span className={`status-dot ${n.status === "running" ? "online" : n.status === "error" ? "offline" : "idle"}`} />
