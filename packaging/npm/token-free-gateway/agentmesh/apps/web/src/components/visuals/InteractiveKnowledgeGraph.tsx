...
const INITIAL_NODES: GraphNode[] = [
  { id: "agent-claude", label: "Claude 3.5 Sonnet", category: "agent", x: 200, y: 140, connections: 4, details: "추론 및 코드 리뷰 에이전트. 2,890건 작업 완료" },
  { id: "agent-gemini", label: "Gemini 1.5 Pro", category: "agent", x: 380, y: 110, connections: 3, details: "실시간 웹 검색 및 장문 리서치 에이전트" },
  { id: "model-llama", label: "Llama-3.3-70B", category: "model", x: 220, y: 280, connections: 3, details: "P2P 엣지 분산 모델 웨이트. Q4 샤딩 구동" },
  { id: "skill-search", label: "InfoMesh Search", category: "skill", x: 440, y: 240, connections: 2, details: "분산 P2P 지식 벡터 색인 도구 (MCP 연동)" },
  { id: "claim-webrtc", label: "CRDT #89: WebRTC MTU", category: "claim", x: 320, y: 210, connections: 5, details: "Society Protocol 검증 지식: 128B MTU 조각화 방지 증명" },
  { id: "human-yeon", label: "Dr. Yeon (Human)", category: "human", x: 120, y: 210, connections: 2, details: "PoH 최종 검증 전문가 노드 (평판 994)" },
];
...
export function InteractiveKnowledgeGraph() {
  ...
  const [selectedNode, setSelectedNode] = useState<GraphNode>(INITIAL_NODES[4] ?? INITIAL_NODES[0]!);
  const [filter, setFilter] = useState<string>("all");

  const getNodeColor = (cat: GraphNode["category"]) => {
    switch (cat) {
      case "agent": return "#e6ff87";
      case "model": return "#38bdf8";
      ...
      <div className="ikg-body">
            ...
            {filteredNodes.map((n) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  {isSelected && (
                    <circle r="22" fill="none" stroke={col} strokeWidth="1.5" strokeDasharray="3 3" className="ikg-node-pulse" />
                  )}
                  <text
                    y={isSelected ? 28 : 22}
                    textAnchor="middle"
        ...
        <div className="ikg-inspector">
          ...
          <div className="radar-kv">
            <span>CATEGORY</span>
            <span style={{ color: getNodeColor(selectedNode.category), textTransform: "uppercase", fontWeight: 700 }}>
