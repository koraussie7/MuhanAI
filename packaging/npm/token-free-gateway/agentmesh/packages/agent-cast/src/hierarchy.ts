...
export class HierarchicalAgentCast {
  private directorPrompt = `You are a Director agent. Decompose the following task into 2-4 independent subtasks.
Return JSON only:
{"task":"...","subtasks":[{"id":"1","prompt":"...","assignedTo":"Researcher"},{"id":"2","prompt":"...","assignedTo":"Analyst"}]}`;
  ...
  private async runDirector(question: string): Promise<DirectorPlan> {
    ...
    try {
      ...
      return {
        ...
        subtasks: Array.isArray(parsed.subtasks) ? parsed.subtasks : [{ id: "1", prompt: question, assignedTo: "Writer" }],
      };
    } catch {
      return {
        task: question,
  ...
  private async runCollaborators(plan: DirectorPlan, question: string): Promise<CollaboratorResult[]> {
    const results: CollaboratorResult[] = [];

    for (const subtask of plan.subtasks) {
      const systemPrompt = this.collaboratorPrompts[subtask.assignedTo] ?? this.collaboratorPrompts.Writer;
      const result = await llmRouter.generate({
