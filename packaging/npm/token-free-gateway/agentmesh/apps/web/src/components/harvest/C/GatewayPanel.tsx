				...
				<tbody>
					{providers.map((p) => (
						<tr key={p.name} className={`hc-gateway-row ${p.status}`}>
							...
							<td>
								<span
									className={`hc-status-dot ${p.status === "healthy" ? "idle" : p.status === "degraded" ? "warm" : "offline"}`}
								/>
...
export function PolicyChain({ chain }: { chain: readonly string[] }) {
	return (
		<section className="hc-panel">
...
export function ApiVault({
	...
	entries: { name: string; quota: number; used: number }[];
}) {
	return (
		<section className="hc-panel">
