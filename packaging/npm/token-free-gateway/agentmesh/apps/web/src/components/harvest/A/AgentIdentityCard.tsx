...
const CAP_ICON: Record<string, string> = {
  Research: "🔍", Analysis: "📊", Coding: "💻", "Web Search": "🌐",
  Summarization: "📝", Review: "✅", "Local Inference": "🧠", Privacy: "🔒",
  Experience: "🎓", Verification: "🛡️",
};

function DIDBadge({ did, verified }: { did: string; verified?: boolean | undefined }) {
  return (
    <span className="did-badge" title={did}>
      <span className="did-badge-dot" aria-hidden>⬢</span>
      <span className="did-badge-text">{did.slice(0, 18)}…</span>
      {verified && <span className="did-badge-verified" title="Verified DID">✓</span>}
    </span>
  );
}
...
export function AgentIdentityCard({ agent, onUse, onConnect }: {
  agent: AgentIdentity;
  onUse?: ((id: string) => void) | undefined;
  onConnect?: ((id: string) => void) | undefined;
}) {
  const statusClass = agent.status === "online" ? "online" : agent.status === "busy" ? "busy" : "offline";
  return (
    <article className="mesh-card identity-card">
      ...
      <div className="discovery-hint">
        ...
        <span className="discovery-value">{agent.a2a ? `${agent.a2a.peers} nodes · ${agent.a2a.verified ? "attested" : "unverified"}` : "local only"}</span>
      </div>

      <div className="mesh-actions">
        <button className="btn-secondary" onClick={() => onConnect?.(agent.id)}>Connect</button>
        <button className="btn-primary" onClick={() => onUse?.(agent.id)}>Use</button>
      </div>
    </article>
  );
}

export function AgentIdentityGrid({ agents, onUse, onConnect, filter }: {
  ...
  const filtered = q ? agents.filter(a => a.name.toLowerCase().includes(q) || a.capabilities.join(" ").toLowerCase().includes(q) || a.did.toLowerCase().includes(q)) : agents;
  return (
    <div className="agent-card-grid">
      {filtered.map(a => <AgentIdentityCard key={a.id} agent={a} onUse={onUse} onConnect={onConnect} />)}
