...
function ProtocolBadge({ protocol }: { protocol: Peer["protocol"] }) {
	const label: Record<Peer["protocol"], string> = {
		webrtc: "WebRTC",
		websocket: "WS",
		memory: "Memory",
		libp2p: "libp2p",
		...
		<div className="peer-canvas">
			<div className="peer-canvas-toolbar">
				<div className="peer-filter-row">
					{(["all", "connected", "connecting", "offline"] as const).map((f) => (
						<button
							key={f}
							className={`policy-chip ${filter === f ? "active" : ""}`}
							onClick={() => setFilter(f)}
						>
			...
			<div className="peer-grid">
				{filtered.map((p) => (
					<article
						key={p.id}
						className={`peer-card ${selectedId === p.id ? "selected" : ""}`}
					...
					>
						...
						<div className="peer-caps">
							{p.capabilities.slice(0, 4).map((c) => (
								<span key={c} className="cap-chip sm">
									{c}
								</span>
							))}
							{p.capabilities.length > 4 && (
								<span className="cap-more">+{p.capabilities.length - 4}</span>
							)}
						</div>
						<div className="peer-card-foot">
							<span className={`peer-status ${p.status}`}>{p.status}</span>
							<span className="peer-connect-hint">
								{p.status === "connected"
									? "● live"
									: p.status === "connecting"
										? "◐ handshake"
...
export function SandboxToolbar({ onAction }: { onAction?: (action: string) => void }) {
	return (
		<div className="sandbox-toolbar">
			...
			<button className="btn-secondary sm" onClick={() => onAction?.("isolate")}>
				Isolate
			</button>
			<button className="btn-secondary sm" onClick={() => onAction?.("inspect")}>
				Inspect
			</button>
			<button className="btn-secondary sm" onClick={() => onAction?.("restart")}>
				Restart
			</button>
...
export function BrowserAgentPanel({
	...
	if (!peer)
		return (
			<div className="browser-agent-panel empty">
				<p className="dash-note">피어를 선택하세요</p>
			</div>
		);
	return (
		<div className="browser-agent-panel">
			...
			<div className="browser-agent-body">
				...
				<div className="kv">
					<span>상태</span>
					<span className={`peer-status ${peer.status}`}>{peer.status}</span>
				</div>
				{peer.latencyMs != null && (
					<div className="kv">
						<span>지연</span>
