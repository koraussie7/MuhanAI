...
import { decryptSessionRoute, encryptSessionPayload, type SessionKey } from "./session-decrypt.js";
...
export interface DaemonHandle {
	...
	dispatchSession(
		req: import("./session-runner.js").SessionRequest,
	): Promise<import("./session-runner.js").SessionResponse>;
...
export function createDaemon(config: DaemonConfig): DaemonHandle {
	...
	return {
		async start() {
			...
			await config.transport.publish(GATEWAY_PUBSUB_TOPIC, encodeMessage(claim.message));
			...
			heartbeatTimer = setInterval(async () => {
				...
				await config.transport.publish(GATEWAY_PUBSUB_TOPIC, encodeMessage(hb.message));
			}, config.heartbeatIntervalMs ?? HEARTBEAT_INTERVAL_MS);

			// 3. Subscribe to inbound session routes.
			unsubscribe = config.transport.subscribe(GATEWAY_PUBSUB_TOPIC, (bytes) => {
				try {
