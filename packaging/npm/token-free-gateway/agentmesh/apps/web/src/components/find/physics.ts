...
/**
 * Upper bound on per-frame velocity magnitude (in world units per tick).
 * Caps the speed vector after damping so a transient force spike — e.g.
 * two co-located nodes' softening-bounded repulsion — can't fling a node
 * off-screen before damping has a chance to bleed energy out.
 */
