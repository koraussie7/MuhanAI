...
import { type A2UIEvent, eventSubscribers } from "./a2ui-routes.js";
...
describe("A2UI API routes", () => {
	...
	it("publishes a surface.ready event when a surface folds to ready", async () => {
		const app = await buildApp({ enableTransport: false, logger: pino({ level: "silent" }) });
		const received: A2UIEvent[] = [];
		const handler = (event: A2UIEvent) => {
			received.push(event);
		};
		eventSubscribers.set("s", new Set([handler]));
		try {
			const response = await app.inject({
				method: "POST",
				url: "/api/a2ui/fold",
	...
	it("does not publish surface.ready when fold is invalid", async () => {
		const app = await buildApp({ enableTransport: false, logger: pino({ level: "silent" }) });
		const received: A2UIEvent[] = [];
		const handler = (event: A2UIEvent) => {
			received.push(event);
