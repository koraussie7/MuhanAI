import { Bot, MessageSquarePlus, ShieldCheck, Users, Zap } from "lucide-react";
...
export function HelpNeeded() {
	...
	async function answer(id: string) {
		try {
			const res = await fetch(`${API}/api/help-needed/${id}/answer`, {
				method: "POST",
			});
			if (res.ok) {
	...
	async function verify(_id: string) {
	...
	return (
		<section id="help" className="help-needed-section">
			<div className="section-head-boxed">
				...
				<h2 className="section-boxed-title">AI가 해결하지 못한 문제에 참여하세요</h2>
				<p className="section-boxed-desc">
					지식 베이스 충돌이나 최신 규정 변경 등으로 AI의 신뢰도가 낮은 문제입니다. 실제 경험을
					공유하고 MHT 보상을 획득하세요.
				</p>
			</div>
					...
					<article className="help-card-boxed" key={item.id}>
						...
						<div className="card-top-row">
							...
							<span className="reward-chip-boxed">
								<Zap size={13} />+{item.reward} Credit
							</span>
						</div>
						...
						<div className="help-metrics-strip">
							<div className="metric-chip">
								...
								<span
									className="metric-chip-val"
									style={{
										color: item.aiConfidence < 0.5 ? "var(--cline-rose)" : "var(--cline-amber)",
									}}
								>
						...
						<div className="help-actions-row">
							...
							<button type="button" className="card-btn-secondary" onClick={() => verify(item.id)}>
								<ShieldCheck size={14} />
								검증하기
							</button>
