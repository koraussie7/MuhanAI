...
import { personalMcpRegistry, PersonalMCP, createPersonalMCP } from "./server";
...
export async function getUserKnowledgeObject(
  userId: string
): Promise<UserKnowledgeObject | null> {
  const store = await getKnowledgeStore();
...
export async function addUserKnowledge(
  ...
  try {
    const { indexKnowledgeNode } = await import(
      "../../knowledge-base/src/index-pipeline"
    );
...
export async function searchUserKnowledge(
  ...
  const { hybridSearch } = await import(
    "../../knowledge-base/src/hybrid-search"
  );
  return hybridSearch({
    userId,
    query,
