...
import { createGateway, encodeMessage, decodeMessage, type SignedPayload, type ProtocolMessage, type MachineClaim, type Heartbeat, type SessionRoute, type SessionAck } from "@agentmesh/gateway";
...
describe("E2E: gateway → muhan-agent session routing", () => {
  ...
  it("claim → heartbeat → session-route → response", async () => {
    ...
    let capturedAck: SessionAck | null = null;
    const unsub = gateway.subscribe((msg) => {
      const m = msg.message;
      if (m.kind === "session-ack") capturedAck = m;
    });
    // Manually feed the claim into the gateway (libp2p wiring is stubbed).
    gateway.publish({
      signature: sign({ signature: new Uint8Array(), from: PEER_ID, message: claimMsg }),
      from: PEER_ID,
      message: claimMsg,
    });
    ...
    gateway.publish({
      signature: sign({ signature: new Uint8Array(), from: PEER_ID, message: route }),
      from: PEER_ID,
      message: route,
    });
    ...
    expect(result.result).toEqual({ items: [{ id: "k_alpha", title: "alpha" }] });
    unsub();
  });
