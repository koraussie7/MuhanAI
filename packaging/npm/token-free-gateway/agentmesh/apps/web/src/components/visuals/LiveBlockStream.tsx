...
const INITIAL_BLOCKS: LedgerBlock[] = [
	{
		blockNumber: 48921,
		hash: "0x8f4a9b...c21",
		contributor: "Seoul-Validator-KR",
		action: "knowledge_verify",
		reward: 250,
		gasTokens: 0,
		timeAgo: "1초 전",
	},
	{
		blockNumber: 48920,
		hash: "0x3e11fa...88d",
		contributor: "Claude-3.5-Analyzer",
	...
	},
	{
		blockNumber: 48919,
		hash: "0x7a992d...14e",
		contributor: "Tokyo-Gpu-Inference",
		action: "compute_inference",
		reward: 180,
		gasTokens: 0,
		timeAgo: "8초 전",
...
export function LiveBlockStream() {
	...
	const getActionBadge = (action: LedgerBlock["action"]) => {
		switch (action) {
			case "knowledge_verify":
				return { label: "검증 승인", color: "#e6ff87" };
			case "compute_inference":
				return { label: "연산 추론", color: "#38bdf8" };
			case "webrtc_relay":
				return { label: "릴레이 라우트", color: "#ffb86b" };
			case "mcp_tool_exec":
				return { label: "MCP 도구 실행", color: "#c084fc" };
		}
	};
		...
		<div className="block-stream-container">
			<div className="block-stream-header">
				...
				<div className="block-stream-stat">
					<span>
						합의 속도: <strong>1.8s / Block</strong>
					</span>
					<span>
						총 채굴 보상: <strong>1,489,200 CR</strong>
					</span>
				</div>
			</div>
			...
			<div className="block-cards-row">
				{blocks.map((b, i) => {
					...
					return (
						<div
							className={`ledger-block-box ${i === 0 ? "new-block" : ""}`}
							key={`${b.blockNumber}-${i}`}
						>
